...
main(int argc, char **argv){
  ...
  int *I0;
	...
	IloNumVarArray2 w(env,I);
	for (int i = 0 ; i < I; i++){
		w[i] = IloNumVarArray (env, T, 0,INF,ILOBOOL);
	}
	IloNumVarArray q(env, T, 0,INF,ILOFLOAT);
  // Objective function
	expr.clear();
	for (int i = 0 ; i < I; i++){
		for (int t = 0 ; t < T; t++){
	...
	for (int t = 1 ; t < T; t++){
		for (int i = 0 ; i < I; i++){
			model.add (w[i][t] <= y[i][t-1]+w[i][t-1]);	
		}
	}

	for (int t = 0 ; t < T-1; t++){
		for (int i = 0 ; i < 3; i++){
			model.add (y[i][t] + q[t] <= 1);	
		}
	}

	for (int t = 0 ; t < T-1; t++){
		for (int i = 0 ; i < 3; i++){
			model.add (w[i][t+1] + w[i][t] <=1 - q[t]);	
		}
	}
